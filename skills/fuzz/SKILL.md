...
2. **Classify** — app bug vs test gap (the four-move decision tree lives in `/ripplo:run`):
   ...
   - **Test gap** — the check failed because a declaration is missing or wrong: an effect the action really has but never declared, a fact scoped too broadly, a world producing an unintended state. Fix the declaration in `.ripplo/`.
