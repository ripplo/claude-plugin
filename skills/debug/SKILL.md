...
2. **Classify** — same decision as above, sharper stakes:
   - **App bug** — the model is right, the app breaks when actions compose this way. Fix the app. File it: `npx ripplo report-bug` — omit BOTH `--run` and `--test` (exploration runs have no server Run row; passing the exploration run id makes the server reject the report). Cite the exploration run id in `--surfaced-by` instead. Kind per the decision tree above (usually `latent_bug` — exploration finds territory no author covered).
