...
A run takes ~30–60s and re-running tells you nothing new unless you've changed something. The run output already names the failed step and the oracle's findings; `.ripplo/debug/<runId>/behavior.jsonl` holds the full causal stream.
...
1. **App bug** — the model describes the promised behavior and the app diverged. Fix the app; never weaken the test to match broken behavior. File it with `npx ripplo report-bug` (kind tree, bar, and fields in `/ripplo:report`).
2. **Strengthen the assertion** — the app is right and the test under-specified the outcome (e.g. a mutation with no backend effect declared). Add the missing `created/updated/deleted` or UI check.
3. **Restrict the `given`** — the expected behavior only holds from a narrower starting state. Tighten this test's world so it always starts in the state its assertions assume.
4. **Split into a new test** — the case excluded by restricting `given` is real behavior the model should cover. Stub a new test with its own world and put it in scope.
...
Multiple failures: pick the most upstream one (world/seed or shared-entity over a test-specific selector), own it through fix and verify, then move on. Verify with `npx ripplo run <test-id>` until green, then bare `npx ripplo run` once so cross-test breakage surfaces. Don't batch edits across tests — when the suite lights up red you can't tell which edit broke what.
...
- **Race** — the action fired before the page settled. Add a `visible(...)` predicate to the prior step's `.expect(...)`.
- **Consistency divergence** — an `Entity.created/updated/deleted` didn't match. The finding names the entity/field and expected-vs-observed:
  - **field-mismatch / absent / ghost-write** → the DB didn't reach the expected state: app dropped/mis-wrote the value (check `network`/`span`), or the assertion expects the wrong value.
  - **"write never landed within the Ns settle budget"** → the app still showed the pre-step value at the deadline — slow write, not wrong. Declare `wait: "slow"` (or `"async"`) on that expectation; don't switch the field to `consistency: "eventual"` (that also tolerates wrong intermediate values).
  - A wrong intermediate value under strict consistency fails fast by design — app bug, not timing.
...
- **View law violation** — `view law never held`, with a `law from:` line naming the originating test. Ripplo generalizes assertions like "at URL X, heading Y is visible" into laws enforced across tests. If your test legitimately reaches that URL in a different state, make the originating assertion conditional: `when([count(Entity).is(0), visible(heading("No items"))])`.
...
- **World / seed wrong** — the starting state isn't what the test assumes. Check the engine impl's `seed`/`read`, not the test.
