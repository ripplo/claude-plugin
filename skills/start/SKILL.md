...
Either way the daemon still owns the local dev session, hooks, and IPC — only where the runs execute changes.
...
3. **`Dev session` red:** spawn the daemon via `Bash` with `run_in_background`, `cwd` = the directory containing `.ripplo/` (workspace root in monorepos). Default is `npx ripplo daemon`; if the user asked for cloud (`/ripplo:start cloud`), spawn `npx ripplo daemon --executor cloud` instead.
